  ...
  extern "C" 
  {
    void axcuda_(int *nx1,int *ny1,int *nz1,int *nelt,int *ldim, double *w, double *u,
      ...
      int e, k, i;
      for (e = 0; e < *nelt; e++) {
        cmxm(dxm1, &m1, &((*um)[e][0]), &m1, (double *) ur, &m2);
        for (k = 0; k < *nz1; k++) {
          cmxm(&((*um)[e][(*nx1)*(*ny1)*k]), &m1, dxtm1, &m1, &us[(*nx1)*(*ny1)*k], &m1);
        }
        cmxm(&((*um)[e][0]), &m2, dxtm1, &m1, (double*)ut, &m1);

        for (i = 0; i < nxyz; i++) {
          double wr = (*gm)[e][i][0]*ur[i] + (*gm)[e][i][1]*us[i] + (*gm)[e][i][2]*ut[i];
          double ws = (*gm)[e][i][1]*ur[i] + (*gm)[e][i][3]*us[i] + (*gm)[e][i][4]*ut[i];
          double wt = (*gm)[e][i][2]*ur[i] + (*gm)[e][i][4]*us[i] + (*gm)[e][i][5]*ut[i];
          ur[i] = wr;
          us[i] = ws;
          ut[i] = wt;
        }

        cmxm(dxtm1, &m1, (double*)ur, &m1, &((*wm)[e][0]), &m2);

        double wa[*nz1][*ny1][*nx1];
        for (k = 0; k < *nz1; k++) {
          cmxm(&us[(*nx1)*(*ny1)*k], &m1, dxm1, &m1, &wa[k][0][0], &m1);
        }
        cadd2(&((*wm)[e][0]), (double*)wa, &m3);

        cmxm((double*)ut, &m2, dxm1, &m1, (double*)wa, &m1);
        cadd2(&((*wm)[e][0]), (double*)wa, &m3);
      }
